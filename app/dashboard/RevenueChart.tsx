    ...
    <div className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 h-full flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500">Havi bontású pénzügyi statisztika</p>
        </div>
        ...
        <div className="flex gap-1 bg-gray-50 dark:bg-gray-950 p-1 rounded-lg border border-gray-100 dark:border-gray-800 self-start sm:self-center">
          {(['successful', 'pending', 'failed'] as RevenueStatus[]).map((status) => {
            ...
            return (
              ...
              >
                <span className={`w-2 h-2 rounded-full ${statusConfig[status].dotColor} ${isActive ? 'animate-pulse' : 'opacity-60'}`} />
        ...
        <div className="text-right h-10 sm:h-auto min-w-[120px]">
          {hoveredIndex !== null ? (
            <div>
              <span className="text-xs text-gray-400 block">{data[hoveredIndex].month}i {statusConfig[activeStatus].label.toLowerCase()}:</span>
              <span className={`text-sm font-bold ${statusConfig[activeStatus].textClass}`}>
                ${data[hoveredIndex][activeStatus].toLocaleString('en-US')}
              </span>
            </div>
          ) : (
            <span className="text-xs text-gray-400 italic leading-10 block">Húzd rá az egeret!</span>
          )}
        </div>
      </div>
      ...
      <div className="relative h-64 lg:h-auto lg:flex-1 mt-4 min-h-[12rem]">
        ...
        <div className="absolute inset-0 flex justify-between items-end pt-4 px-2 gap-3">
          {data.map((item, index) => {
            ...
            const heightPercent = currentAmount > 0 ? Math.max(calculatedPercent, 4) : 0;
