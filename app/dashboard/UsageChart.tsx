...
export default function UsageChart({ chartData = [] }: UsageChartProps) {
  ...
  const config = {
    all: { label: 'Összesen', color: 'bg-blue-500', stroke: '#3B82F6', grad: 'colorAll' },
    success: { label: 'Sikeres', color: 'bg-green-500', stroke: '#10B981', grad: 'colorSuccess' },
    pending: { label: 'Függőben', color: 'bg-amber-500', stroke: '#F59E0B', grad: 'colorPending' },
    failed: { label: 'Meghiúsult', color: 'bg-red-500', stroke: '#EF4444', grad: 'colorFailed' },
  };
        ...
        <div className="flex flex-wrap gap-1 bg-gray-50 dark:bg-gray-900 p-1 rounded-lg border border-gray-100 dark:border-gray-700">
          ...
          <button
            ...
            className={`text-xs font-semibold px-2.5 py-1.5 rounded-md transition-all duration-200 flex items-center gap-1.5 ${
              showAll ? 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm' : 'text-gray-400 opacity-50'
            }`}
          >
          ...
          <button
            ...
            className={`text-xs font-semibold px-2.5 py-1.5 rounded-md transition-all duration-200 flex items-center gap-1.5 ${
              showSuccess ? 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm' : 'text-gray-400 opacity-50'
            }`}
          >
          ...
          <button
            ...
            className={`text-xs font-semibold px-2.5 py-1.5 rounded-md transition-all duration-200 flex items-center gap-1.5 ${
              showPending ? 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm' : 'text-gray-400 opacity-50'
            }`}
          >
          ...
          <button
            ...
            className={`text-xs font-semibold px-2.5 py-1.5 rounded-md transition-all duration-200 flex items-center gap-1.5 ${
              showFailed ? 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm' : 'text-gray-400 opacity-50'
            }`}
          >
          ...
          <AreaChart data={data} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
            ...
            <defs>
              ...
              <linearGradient id={config.all.grad} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={config.all.stroke} stopOpacity={0.15}/>
                <stop offset="95%" stopColor={config.all.stroke} stopOpacity={0}/>
              </linearGradient>
              {/* Zöld (Sikeres) */}
              <linearGradient id={config.success.grad} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={config.success.stroke} stopOpacity={0.15}/>
                <stop offset="95%" stopColor={config.success.stroke} stopOpacity={0}/>
              </linearGradient>
              {/* Sárga (Függőben) */}
              <linearGradient id={config.pending.grad} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={config.pending.stroke} stopOpacity={0.15}/>
                <stop offset="95%" stopColor={config.pending.stroke} stopOpacity={0}/>
              </linearGradient>
              {/* Vörös (Meghiúsult) */}
              <linearGradient id={config.failed.grad} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={config.failed.stroke} stopOpacity={0.15}/>
                <stop offset="95%" stopColor={config.failed.stroke} stopOpacity={0}/>
              </linearGradient>
            </defs>
